...
def rebuild_predictions() -> dict:
    ...
    accepted = [r for r in records if r.get('status') in {'accepted', 'arrived'}]
    ...
    current_start = _now()
    accepted_in_service = accepted[0] if accepted else None
    if accepted_in_service and accepted_in_service.get('accepted_at'):
        accepted_at = _parse_iso(accepted_in_service.get('accepted_at')) or current_start
        current_start = max(accepted_at, current_start)
    elif accepted_in_service and accepted_in_service.get('arrived_at'):
        arrived_at = _parse_iso(accepted_in_service.get('arrived_at')) or current_start
