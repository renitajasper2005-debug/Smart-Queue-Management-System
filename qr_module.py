...
def generate_qr(record_id: str, base_url: str = "http://localhost:5000") -> str:
    """
    Generate a QR code that encodes the admin scan URL for the given record ID.
    Returns the relative path to the saved image: 'static/qrcodes/<id>.png'
    """
    check_in_url = f"{base_url}/admin/scan/{record_id}"
    qr = qrcode.QRCode(
        version=1,
    ...
    # Use a high-contrast QR so phone cameras and browser scanners read it reliably.
